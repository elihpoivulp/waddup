...
class Post extends Model
{
    ...
    public static function getPostsForScroll(?int $id = null): bool|array
    {
        ...
        $params = [':id' => $id];
        if (is_null($id)) {
            $sql = 'select p.*, u.username as writer, u.photo as user_photo, ifnull((select count(id) from comments where post_id = p.id), 0) as comments_count from posts p join users u on u.id = p.user_id where p.expired = 0 order by p.id desc limit 3';
            $params = [];
        }
